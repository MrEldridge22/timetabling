...
all_subjects = pd.read_csv("code_check_csvs/all_tdf_export.csv")
# Remove Unwanted Columns
all_subjects.drop(all_subjects.iloc[:, 0:2], inplace=True, axis=1)
all_subjects.drop(all_subjects.iloc[:, 2:], inplace=True, axis=1)
all_subjects.dropna(inplace=True)  # Remove incomplete Data

# Rename column headings
all_subjects.rename(columns={'Subject Name': 'sub_name', 'Subject Code': 'sub_code'}, inplace=True)
